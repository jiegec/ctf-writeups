...
The idea is to call functions with parens using `[[]for[a]in[[b]]]` primitive. Previously in [pyjail cheatsheet](https://shirajuki.js.org/blog/pyjail-cheatsheet/) we know that we can do `[[]for a in[b]]`, but the spaces are required. The extra `[]` layer makes it work without spaces.
