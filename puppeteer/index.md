# Redbud Puppeteer CTF Writeups

[Puppeteer](https://thuctf.redbud.info/) 是 Redbud 组织的每周 CTF 活动。你可以在我的 [存档网站](https://ctf.jia.je/) 中做这些题目。

## 目录

### Week 0

- [Easy Random 1](./week0/easy-random-1.md): 攻击 glibc 随机数生成器
- [Easy Random 2](./week0/easy-random-2.md): 攻击 Python 随机数生成器
- [The Old Gods Are Whispering](./week0/the-old-gods-are-whispering.md): 音频隐写

### Week 1

- [Easy Pyjail 1](./week1/easy-pyjail-1.md): 简单的 Python jail（禁用 builtins）
- [Easy Random 3](./week1/easy-random-3.md): 使用 gf2bv 攻击 Python 随机数生成器
- [Easy RE 1](./week1/easy-re-1.md): 简单的可逆密码的逆向工程
- [Rosetta 3](./week1/rosetta-3.md): 三种字符编码（使用 dcode.fr 解码）
- [RSA All in One](./week1/rsa-all-in-one.md): 五种典型的 RSA 攻击

### Week 2

- [Easy Random 4](./week2/easy-random-4.md): 攻击 LCG 随机数生成器
- [ECDSA Nonce Reuse](./week2/ecdsa-nonce-reuse.md): 攻击复用 Nonce 的 ECDSA

### Week 3

- [Easy Random 5](./week3/easy-random-5.md): 攻击 Xoshiro256starstar 随机数生成器
- [RSA Partial Factorization](./week3/rsa-partial-factorization.md): 使用 Coppersmith 算法攻击 RSA

### Week 4

- [Easy Random 6](./week2/easy-random-6.md): 攻击 Truncated LCG 随机数生成器
